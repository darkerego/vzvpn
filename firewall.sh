...
staticIP="1.2.3.6" # Only allow ssh from inside the tunnel and
####also  allow from one static IP incase the tunnel dies.
########---END-VARIABLES--#########
...
#### (we should never be incoming from/to our own IP)####
...
$IPT -A INPUT -i venet0 -s $staticIP -d $EXTIP -p tcp -m tcp --source-port 513:65535 --dport $sshPRT -j ACCEPT
...
#### Drop incoming on my 2ndary (outgoing) ip ####
...
#### Drop Windows BS (Too much logging) ####
